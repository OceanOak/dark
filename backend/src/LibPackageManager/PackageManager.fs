...
module PackageCache =
  ...
  let private getAllDefinitions (kind: string) : Task<List<uuid * byte[]>> =
    task {
      try
        ...
        debuG $"Found {results.Length} items for {kind}" ()
        return results
      with ex ->
        debuG $"Database error for {kind}" ex
        return raise ex
    }

  let private getDefinitionFromDB (kind: string) (id: uuid) : Task<Option<byte[]>> =
    task {
      try
        use connection = new SqliteConnection("Data Source=sql-db/test.db")
        do! connection.OpenAsync()
